...
Definition io_event {width: Z}{BW: Bitwidth width}{word: word.word width} : Type :=
  (String.string * list word) * (list word).
...
End WithIOEvent. (*maybe extend this to the end?*)
  ...
  Definition ExtSpec{width: Z}{BW: Bitwidth width}{word: word.word width} :=
  (* Given a trace of what happened so far,
     the given-away memory, an action label and a list of function call arguments, *)
  io_trace -> String.string -> list word ->
...
Module ext_spec.
  Class ok{width: Z}{BW: Bitwidth width}{word: word.word width}
          {ext_spec: ExtSpec}: Prop :=
  {
    weaken :> forall t act args,
        Morphisms.Proper
    ...
    intersect: forall t a args,
      ext_spec t a args (fun resvals klist =>
                                 forall mid, ext_spec t a args mid ->
                                             mid resvals klist);
  }.
End ext_spec.
...
Module exec. Section WithEnv.
  ...
  Implicit Types post : trace -> io_trace -> locals -> metrics -> Prop. (* COQBUG(unification finds Type instead of Prop and fails to downgrade *)
  Implicit Types fpost : PickSp -> trace -> io_trace -> locals -> metrics -> Prop.
   ...
   *)
  Inductive exec :
    cmd -> trace -> io_trace -> locals -> metrics ->
    (trace -> io_trace -> locals -> metrics -> Prop) -> Prop :=
  | skip
    k t l mc post
    (_ : post k t l mc)
    : exec cmd.skip k t l mc post
  | set x e
    l mc post
    k t v mc' k' (_ : eval_expr l e mc k = Some (v, mc', k'))
    (_ : post k' t (map.put l x v) (addMetricInstructions 1
                                      (addMetricLoads 1 mc')))
    : exec (cmd.set x e) k t l mc post
  | unset x
    k t l mc post
    (_ : post k t (map.remove l x) mc)
    : exec (cmd.unset x) k t l mc post
  | stackalloc x n body
    k t l mc post
    (_ : Z.modulo n (bytes_per_word width) = 0)
    (_ : forall a,
        (salloc_det = true -> a = pick_sp k) ->
        exec body (consume_word a :: k) t (map.put l x a) (addMetricInstructions 1 (addMetricLoads 1 mc))
          (fun k' t' l' mc' =>
              post k' t' l' mc'))
    : exec (cmd.stackalloc x n body) k t l mc post
  | seq c1 c2
    k t l mc post
    mid (_ : exec c1 k t l mc mid)
    (_ : forall k' t' l' mc', mid k' t' l' mc' -> exec c2 k' t' l' mc' post)
    : exec (cmd.seq c1 c2) k t l mc post
  | while_false e c
    k t l mc post
    v mc' k' (_ : eval_expr l e mc k = Some (v, mc', k'))
    (_ : word.unsigned v = 0)
    (_ : post (leak_bool false :: k') t l (addMetricInstructions 1
                                                (addMetricLoads 1
                                                   (addMetricJumps 1 mc'))))
    : exec (cmd.while e c) k t l mc post
  | while_true e c
      k t l mc post
      v mc' k' (_ : eval_expr l e mc k = Some (v, mc', k'))
      (_ : word.unsigned v <> 0)
      mid (_ : exec c (leak_bool true :: k') t l mc' mid)
      (_ : forall k'' t' l' mc'', mid k'' t' l' mc'' ->
                                      exec (cmd.while e c) k'' t' l' (addMetricInstructions 2
                                                                           (addMetricLoads 2
  ...
  | interact binds action arges
      ...
      mid (_ : ext_spec t action args mid)
      (_ : forall resvals klist, mid resvals klist ->
          ...
          post (leak_list klist :: k')%list (((action, args), resvals) :: t) l'
            (addMetricInstructions 1
               (addMetricStores 1
                  (addMetricLoads 2 mc'))))
    : exec (cmd.interact binds action arges) k t l mc post
  .

  Definition state : Type := trace * io_trace * locals * metrics.
  ...
  | end_interact (binds : list String.string) (action : String.string) (args : list word) (resvals klist : list word).
  ...
  Inductive step :
    scmd -> trace -> io_trace -> locals -> metrics ->
    scmd -> trace -> io_trace -> locals -> metrics -> Prop :=
  | set_step x e
      l mc
      k t v mc' k' (_ : eval_expr l e mc k = Some (v, mc', k'))
    : step (sset x e) k t l mc
        sskip k' t (map.put l x v) (ami 1 (aml 1 mc'))
  | unset_step x
    k t l mc
    : step (sunset x) k t l mc
        sskip k t (map.remove l x) mc
  | stackalloc_step x n body a
      k t l mc
      (_ : Z.modulo n (bytes_per_word width) = 0)
      (_ : salloc_det = true -> a = pick_sp k)
    : step (sstackalloc x n body) k t l mc
        (sseq body (end_stackalloc n a)) (consume_word a :: k) t (map.put l x a) (ami 1 (aml 1 mc))
  | stackalloc_end_step n a
      k t l mc
    : step (end_stackalloc n a) k t l mc
        sskip k t l mc
  | seq_step c1 c2
      k t l mc
      c1' k' t' l' mc'
    (_ : step c1 k t l mc c1' k' t' l' mc')
    : step (sseq c1 c2) k t l mc
        (sseq c1' c2) k' t' l' mc'
  | seq_done_step c2
      k t l mc
    : step (sseq sskip c2) k t l mc
        c2 k t l mc
  | while_false_step e c
      k t l mc
      v mc' k' (_ : eval_expr l e mc k = Some (v, mc', k'))
      (_ : word.unsigned v = 0)
    : step (swhile e c) k t l mc
         sskip (leak_bool false :: k') t l (ami 1 (aml 1 (amj 1 mc')))
  | while_true_step e c
      k t l mc post
      v mc' k' (_ : eval_expr l e mc k = Some (v, mc', k'))
      (_ : word.unsigned v <> 0)
    : step (swhile e c) k t l mc
        (sseq c (sseq jump_back (swhile e c))) (leak_bool true :: k') t l mc'
  | jump_back_step
      k t l mc
    : step jump_back k t l mc
        sskip k t l (ami 2 (aml 2 (amj 1 mc)))
  | interact_step binds action arges
      k t l mc
      args mc' k' (_ : evaluate_call_args_log l arges mc k = Some (args, mc', k'))
      (_ : ext_spec t action args (fun _ _ => True))
      resvals klist
      (_ : forall mid, ext_spec t action args mid -> mid resvals klist)
    : step (sinteract binds action arges) k t l mc
           (end_interact binds action args resvals klist) k' t l mc'
  | end_interact_step (binds : list String.string) (action : String.string) (args : list word)
      (k' : trace) (mc' : MetricLog) (resvals klist : list word)
      k t l mc
      l' (_ : map.putmany_of_list_zip binds resvals l = Some l')
    : step (end_interact binds action args resvals klist) k t l mc
        sskip (leak_list klist :: k)%list (((action, args), resvals) :: t) l' (ami 1 (ams 1 (aml 2 mc))).

  Definition sstate : Type := scmd * trace * io_trace * locals * metrics.
  Definition get_scmd (st : sstate) : scmd :=
    let '(s, k, t, l, mc) := st in s.

  Definition other_inclusion st : sstate :=
    let '(s, k, t, l, mc) := st in
    (inclusion s, k, t, l, mc).

  Definition state_step st st' : Prop :=
    let '(s, k, t, l, mc) := st in
    let '(s', k', t', l', mc') := st' in
    step s k t l mc s' k' t' l' mc'.
  ...
  | good_stuck_stackalloc : forall k t l mc x n body,
      Z.modulo n (bytes_per_word width) = 0 ->
      state_stuck (sstackalloc x n body, k, t, l, mc) ->
      good_stuck (sstackalloc x n body, k, t, l, mc)
  | good_stuck_interact : forall k t l mc binds action arges args mc' k',
      evaluate_call_args_log l arges mc k = Some (args, mc', k') ->
      ext_spec t action args (fun _ _ => True) ->
      state_stuck (sinteract binds action arges, k, t, l, mc) ->
      good_stuck (sinteract binds action arges, k, t, l, mc)
  | good_stuck_end_interact : forall k t l mc binds action args resvals klist,
      ...
      (exists l', map.putmany_of_list_zip binds resvals l = Some l') ->
      state_stuck (end_interact binds action args resvals klist, k, t, l, mc) ->
      good_stuck (end_interact binds action args resvals klist, k, t, l, mc)
  | good_stuck_seq : forall s1 s2 k t l mc,
    good_stuck (s1, k, t, l, mc) ->
    good_stuck (sseq s1 s2, k, t, l, mc).

  Definition state_satisfies post st : Prop :=
    (let '(s, k, t, l, mc) := st in s = sskip /\ post k t l mc) \/
      good_stuck st.
  ...
  | bprefix : forall s1 s2 k t l mc,
      prefix (s1, k, t, l, mc) (sseq s1 s2, k, t, l, mc).
  ...
  Definition first_part st : option sstate :=
    match st with
    | Some (sseq s1 s2, k, t, l, mc) => Some (s1, k, t, l, mc)
  ...
  Ltac destr_sstate st :=
    ...
    let mc := fresh "mc" in
    let Ef := fresh "Ef" in
    destruct st as [ [ [ [s k] t] l] mc] eqn:Ef.
    ...
    - destruct (execution_of_first_part f n) as [s_s2|] eqn:Ebig.
      ...
      + cbv [step_ostate] in H.
        ...
        -- left. cbv [step_ostate]. simpl. rewrite Ebig. rewrite E'. simpl.
           destruct s0_1; try assumption. inversion H11.
      ...
      + right. left. destruct H as [Hp1 Hp2]. split.
        -- destruct (f n) as [st|]; try solve [destruct Hp1]. destr_sstate st.
           ...
           eexists (_, _, _, _, _). eapply seq_step. eassumption.
  ...
  Lemma satisfies_weaken f post1 post2 :
    (forall k t l mc, post1 k t l mc -> post2 k t l mc) ->
    satisfies f post1 ->
    satisfies f post2.
  Proof.
  ...
  Definition append s' (st : sstate) :=
    let '(s, k, t, l, mc) := st in (sseq s s', k, t, l, mc).

  Lemma second_comes_after_first s1 s2 k t l mc k' t' l' mc' f i :
    f O = Some (sseq s1 s2, k, t, l, mc) ->
    possible_execution f ->
    execution_of_first_part f i = Some (sskip, k', t', l', mc') ->
    f (S i) = Some (s2, k', t', l', mc').
  Proof.
    intros H1 H2 H3.
    ...
    - fwd. destruct (f i) as [st'|]; [|destruct H2p0]. inversion H; clear H; subst.
      exfalso. apply H2p0. eexists (_, _, _, _, _). eapply seq_done_step.
    - fwd. congruence.
  Qed.
  
  Lemma build_seq s1 s2 k t l mc post :
    (forall (f : nat -> _),
        f O = Some (s1, k, t, l, mc) ->
        possible_execution f ->
        satisfies f (fun k' t' l' mc' =>
                       forall (g : nat -> _),
                         g O = Some (s2, k', t', l', mc') ->
  ...
  Definition execute_with_tail (f : nat -> option sstate) (s2 : scmd) (n : nat) : option sstate :=
    match f n with
    | Some (s, k, t, l, mc) => Some (sseq s s2, k, t, l, mc)
    | None => None
    end.
  ...
  Lemma comes_after_seq s k t l mc s' k' t' l' mc' s2 :
    comes_after (s', k', t', l', mc') (s, k, t, l, mc) ->
    comes_after (sseq s' s2, k', t', l', mc') (sseq s s2, k, t, l, mc).
  Proof. apply (comes_after_seq' (s, k, t, l, mc) (s', k', t', l', mc')). Qed.
    
  Lemma invert_seq s1 s2 k t l mc post :
    (forall (f : nat -> _),
        f O = Some (sseq s1 s2, k, t, l, mc) ->
        possible_execution f ->
        satisfies f post) ->
    (forall (f : nat -> _),
        ...
        possible_execution f ->
        satisfies f (fun k' t' l' mc' =>
                       comes_after (s2, k', t', l', mc') (sseq s1 s2, k, t, l, mc) /\
                         forall (g : nat -> _),
                           g O = Some (s2, k', t', l', mc') ->
                           possible_execution g ->
                           satisfies g post)).
...
(*is excluded middle really necessary here? intuitively it seems like the only option...
      ...
      the execution of (s1, k, t, l, mc) terminates, then I wouldn't know ahead of time what 
      would be the (sseq sskip s2, k', t', l', mc') state that the sseq s1 s2 eventually ends 
      up in, so I'd then need to have some map (s2, k', t', l', mc') |-> g, where g is a 
    ...
    { enough (H' : o2 comes_after (f n) (Some (s1, k, t, l, mc)) \/ f n = Some (s1, k, t, l, mc)).
      ...
      - left. eassert (hyp : _). 2: specialize (IHn hyp); clear hyp.
        ...
        + destruct (f O) as [st|]; try solve [destruct IHn].
          ...
          -- apply t_step. instantiate (1 := (_, _, _, _, _)). eassumption.
          -- assumption.
    ...
    { exfalso. apply PeanoNat.Nat.leb_le in E1.
      ...
      - specialize (Hs1 b). destruct (Nat.eqb b n) eqn:E2.
        ...
        + apply PeanoNat.Nat.eqb_neq in E2. specialize (Hs1 ltac:(lia)).
          ...
          apply good_stuck_stuck in H. apply H. eexists (_, _, _, _, _). eapply seq_step.
          eassumption. }
    apply H.
  Qed.
  ...
  Context {word_ok: word.ok word} {ext_spec_ok: ext_spec.ok ext_spec}.

  Lemma exec_to_step (s : cmd) k t l mc post :
    exec s k t l mc post ->
    forall (f : nat -> _),
      f O = Some (inclusion s, k, t, l, mc) ->
      possible_execution f ->
      satisfies f post.
  Proof.
    intros H. induction H.
    ...
    - intros f HO HS. assert (HSO := HS O). destruct HSO as [HSO | [HSO | HSO] ].
      ...
      + exfalso. cbv [stuck_ostate] in HSO. rewrite HO in HSO. destruct HSO as [HSO _].
        apply HSO. eexists (_, _, _, _, _). econstructor; eassumption.
      + fwd. congruence.
    - intros f HO HS. assert (HSO := HS O). destruct HSO as [HSO | [HSO | HSO] ].
      + exists (S O). cbv [step_ostate state_step] in HSO. rewrite HO in HSO.
        destruct (f (S O)) as [st'|]; [|destruct HSO]. destr_sstate st'.
    ...
    - intros f HO HS. simpl in HO. clear H0. assert (HSO := HS O).
      ...
      + cbv [step_ostate state_step] in HSO. rewrite HO in HSO.
        ...
        simpl. intros.
        specialize (H5 O). cbv [step_ostate stuck_ostate state_step] in H5.
        rewrite H4 in *. clear H4.
        destruct H5 as [H5 | [H5 | H5]].
        -- destruct (g (S O)) as [st'|] eqn:Eg1; [|destruct H5]. destr_sstate st'.
           inversion H5. subst. fwd.
           subst. eexists (S O). rewrite Eg1. left. auto.
        -- exfalso. apply H5. clear H5. fwd. eexists (_, _, _, _, _).
           econstructor; eauto.
        -- fwd. congruence.
    ...
    - intros f HO HS. assert (HSO := HS O). destruct HSO as [HSO | [HSO | HSO] ].
      ...
      + exfalso. cbv [stuck_ostate] in HSO. destruct HSO as [HSO _]. rewrite HO in HSO.
        apply HSO. clear HSO. eexists (_, _, _, _, _). econstructor; eauto.
      + fwd. congruence.
    - intros f HO HS. assert (HSO := HS O). destruct HSO as [HSO | [HSO | HSO] ].
      + cbv [step_ostate state_step] in HSO. rewrite HO in HSO.
        destruct (f (S O)) as [st'|] eqn:Est'; [|destruct HSO]. destr_sstate st'.
        ...
        -- cbv [step_ostate state_step] in HgSO. rewrite HgO in HgSO.
           ...
           inversion HgSO. subst. inversion H18. subst. assert (HgSSO := HgS (S O)).
           ...
           ++ exfalso. cbv [stuck_ostate] in HgSSO. rewrite Eg1 in HgSSO. apply HgSSO.
              eexists (_, _, _, _, _). eapply seq_done_step.
           ++ fwd. congruence.
        -- exfalso. cbv [stuck_ostate] in HgSO. rewrite HgO in HgSO. destruct HgSO as [HgSO _].
           apply HgSO. eexists (_, _, _, _, _). eapply seq_step. econstructor.
        -- fwd. congruence.
      + exfalso. destruct HSO as [HSO _]. rewrite HO in HSO. apply HSO.
        eexists (_, _, _, _, _). eapply while_true_step; eauto.
      + fwd. congruence.
    - destruct ext_spec_ok.
      ...
      + cbv [step_ostate state_step] in HSO. rewrite HO in HSO.
        ...
        -- cbv [step_ostate state_step] in HSSO. rewrite Est' in HSSO.
           ...
           exists (S (S O)). rewrite Est''. left. intuition. Check H1.
           specialize H16 with (1 := H0). apply H1 in H16. fwd.
           unify_eval_exprs. assumption.
        -- exists (S O). rewrite Est'. right. destruct HSSO as [HSSO _]. rewrite Est' in HSSO.
           econstructor.
           ++ apply H16 in H0. apply H1 in H0. fwd. eauto.
           ++ assumption.
        -- fwd. congruence.
      + assert (step_or_not :=
                  em (exists resvals klist,
                          forall mid',
                            ext_spec t action args mid' ->
                            mid' resvals klist)).
        destruct step_or_not as [step | not_step].
        -- fwd. assert (Hmid := step _ H0). apply H1 in Hmid. fwd.
           exfalso. destruct HSO as [HSO _]. rewrite HO in HSO. apply HSO.
           eexists (_, _, _, _, _). econstructor; eauto.
        ...
        -- exists O. rewrite HO. right. econstructor; eauto.
           { eapply weaken. 2: eassumption. cbv [pointwise_relation Basics.impl].
             intros * Hmid. apply H1 in Hmid. fwd. eauto. }
           intros H'. apply not_step. clear not_step. fwd. cbv [state_step step_ostate] in H'.
           destr_sstate st'. inversion H'. subst.
           unify_eval_exprs.
           assert (Hmid := H16 _ H0).
           eexists. eexists. intuition eauto. eapply H16. apply H2.
      + fwd. congruence.
  Qed.
  ...
  Lemma prefix_wf :
    well_founded prefix.
  Proof.
    cbv [well_founded]. intros. destr_sstate a. subst. revert k t l mc. induction s.
  ...
  Lemma steps_Acc'' s k t l mc post :
    (forall (f : nat -> option (scmd * _ * _ * _ * _)),
        f O = Some (inclusion s, k, t, l, mc) ->
        possible_execution f ->
        satisfies f post) ->
    Acc comes_right_after (inclusion s, k, t, l, mc).
  Proof.
    intros. enough (Acc (lift (o2 comes_right_after) Some) (inclusion s, k, t, l, mc)).
    { apply H0. }
    apply lifted_Acc.
    ...
    - assert (H'' : let '(s, k, t, l, mc) := st in
                    exists s' k' t' l' mc',
                      step s k t l mc s' k' t' l' mc').
      { subst. do 5 eexists. eassumption. }
      rewrite <- Ef in *. clear Ef Est H'. induction H1.
      + apply H2. clear H2. fwd. eexists (_, _, _, _, _). eassumption.
      + apply H3. clear H3. fwd. eexists (_, _, _, _, _). eassumption.
      + apply H2. clear H2. fwd. eexists (_, _, _, _, _). eassumption.
      + apply IHgood_stuck. clear IHgood_stuck. fwd. inversion H''; subst.
        -- do 5 eexists. eassumption.
        -- inversion H1.
  Qed.
  ...
  Lemma steps_Acc' s k t l mc post :
    (forall (f : nat -> option (scmd * _ * _ * _ * _)),
        ...
        possible_execution f ->
        satisfies f post) ->
    Acc comes_after_or_repeated_prefix (inclusion s, k, t, l, mc).
  Proof.
  ...
  Lemma steps_Acc s k t l mc post :
    (forall (f : nat -> option (scmd * _ * _ * _ * _)),
        ...
        possible_execution f ->
        satisfies f post) ->
    Acc lifted_comes_after_or_repeated_prefix (s, k, t, l, mc).
  Proof.
  ...
  Lemma satisfies_short f post i k t l mc :
    possible_execution f ->
    satisfies f post ->
    f i = Some (sskip, k, t, l, mc) ->
    post k t l mc.
  Proof.
  ...
  Lemma weaken: forall s k t l mc post1,
      exec s k t l mc post1 ->
      forall post2: _ -> _ -> _ -> _ -> Prop,
        (forall k' t' l' mc', post1 k' t' l' mc' -> post2 k' t' l' mc') ->
        exec s k t l mc post2.
  Proof.
  ...
  Lemma step_to_exec s k t l mc post :
    (forall (f : nat -> _),
        f O = Some (inclusion s, k, t, l, mc) ->
        possible_execution f ->
        satisfies f post) ->
  ...
  Proof.
    ...
    eapply (@Fix_F _ _ (fun x => let '(_, _, _, _, _) := x in _) _ _ H').
    Unshelve. simpl. clear -em choice word_ok ext_spec_ok. intros. destr_sstate x. subst.
    intros post Hsat.
    (* there is a possible execution *)
    assert (Hposs := possible_execution_exists (Some (inclusion s, k, t, l, mc))).
    ...
    - assert (HpossO := Hposs O). destruct HpossO as [HpossO|[HpossO|HpossO]].
      2: { cbv [stuck_ostate] in HpossO. destruct HpossO as [HpossO _].
           ...
           inversion HpossO. subst. econstructor; eauto. intros. exfalso. apply H7.
           eexists (_, _, _, _, _). econstructor; eauto. }
      2: { fwd. congruence. }
      ...
      econstructor; eauto. clear a f HfO Hposs Hsatf Hposs HpossO H12.
      intros. eset (st2 := (_, _, _, _, _)).
      assert (Xs := X st2).
      eassert (lt : _). 2: specialize (Xs lt); clear lt.
      { cbv [lifted_comes_after_or_repeated_prefix lift]. simpl. eapply t_trans.
        2: { apply t_step. right. instantiate (1 := (_, _, _, _, _)).
             apply t_step. econstructor; eauto. }
        apply t_step. left. apply t_step. constructor. }
      simpl in Xs.
      eassert (Hind' : forall g, g O = Some (_, _, _, _, _) -> possible_execution g -> satisfies g post).
      { intros g HgO Hpossg. eset (Sg := fun n => match n with
        ...
        { simpl in Hsat. destruct Hsat as [Hsat|Hsat].
          ...
          - inversion Hsat. subst. exfalso. apply H8. clear H8.
            eexists (_, _, _, _, _). econstructor; eassumption. }
        exists i. apply Hsat. }
      ...
      inversion Hpossg'O. subst. eapply satisfies_short; eauto.
    - clear f HfO Hposs Hsatf.
      assert (Xs1 := X (s1, k, t, l, mc)). eassert (lt : _). 2: specialize (Xs1 lt); clear lt.
      ...
      fold sstate in *.
      assert (Xs2 := X (s2, k', t', l', mc')). eassert (lt: _). 2: specialize (Xs2 lt); clear lt.
    ...
    - assert (HpossO := Hposs O). destruct HpossO as [HpossO|[HpossO|HpossO]].
      ...
      + (*I'm kind of inlining the whole proof for the seq case here, plus some more stuff...
        ...
        { intros n. destruct n as [|n].
          - left. cbv [step_ostate state_step]. simpl. instantiate (1 := Some (_, _, _, _, _)).
    ...
    - destruct ext_spec_ok.
      ...
      assert (stuck_enough: o1 good_stuck (f O) -> exec (cmd.interact binds action args) k t l mc post).
      { clear HpossO. intros Hstuck. rewrite HfO in Hstuck. inversion Hstuck. subst.
        econstructor. 2: eapply intersect. (*not H9*) all: try eassumption.
        simpl. intros * H. exfalso. apply H8. eexists (_, _, _, _, _).
        econstructor; eassumption. }
      ...
      inversion HpossO. subst. econstructor. 2: eapply intersect. all: try eassumption.
      rewrite HfO in stuck_enough. simpl.
      clear f HfO Hposs Hsatf Hposs Ef1 resvals klist HpossO H13.
      intros * H. eset (f1 := (_, _, _, _, _)).
      ...
      2: { destruct HpossSO as [HpossSO _]. eapply satisfies_stuck_good in HpossSO; try eassumption.
           ...
           exfalso. apply H11. clear H11. eexists (_, _, _, _, _). econstructor; eassumption. }
  ...
  Proof.
    ...
    { intros [H1 H2]. revert H2. induction H1; intros; fwd; try (econstructor; eauto).
      intros _.
      replace (consume_word a :: k) with ((consume_word a :: nil) ++ k) in H2p0 by reflexivity.
      apply app_inv_tail in H2p0. subst. simpl in H2p1. inversion H2p1. subst.
      simpl in H4. specialize (H4 I). inversion H4. subst. reflexivity. }
  Qed.
  ...
  Proof.
    ...
    all: try (exfalso; apply H1; eexists (_, _, _, _, _); econstructor; solve [eauto]).
    constructor. apply IHstep. intros H'. apply H1. destruct H' as [st' H'].
    destr_sstate st'. eexists (_, _, _, _, _). eapply seq_step. eassumption.
  Qed.
  ...
  Lemma nondet_to_det {pick_sp : PickSp} s k t l mc fpost f :
    excluded_middle ->
    f O = Some (s, k, t, l, mc) ->
    (possible_execution_nondet f ->
     satisfies_nondet f (fun k' t' l' mc' =>
                           exists k'',
                             k' = k'' ++ k /\
                               (compat (fun k_ => consume_word (pick_sp (rev k_ ++ k))) (List.rev k'') ->
                                fpost pick_sp k' t' l' mc'))) ->
    possible_execution_det f ->
  ...
  Lemma det_to_nondet_terminates {pick_sp : PickSp} s k t l mc fpost f :
    f O = Some (s, k, t, l, mc) ->
    possible_execution_nondet f ->
    ...
    (possible_execution_det (pick_sp := pick_sp') f ->
     satisfies_det (pick_sp := pick_sp') f (fpost pick_sp')) ->
    exists i,
      match (f i) with
      | Some st => good_stuck false st \/ get_scmd st = sskip
      | None => False
      end.
  Proof.
    ...
    destruct H as [H|H].
    + (*both succeed*) fwd. right. reflexivity.
    + (*both good stuck*) left. assert (Hnone: f (S n) = None).
  ...
  Lemma det_to_nondet' {pick_sp : PickSp} f s k t l mc :
    f O = Some (s, k, t, l, mc) ->
    possible_execution_nondet f ->
    (forall i,
      match f i with
    ...
    possible_execution_det f.
  Proof.
    intros H1 H2 H3. intros i. specialize (H2 i). destruct H2 as [H2 | [H2 | H2]].
    - assert (H3i := H3 i). assert (H3Si := H3 (S i)). clear H3. left. cbv [step_ostate] in *.
      destruct (f i) as [fi|]; [|destruct H2].
      ...
      specialize (H5 I). inversion H5. subst. rewrite app_nil_r. rewrite rev_involutive. reflexivity.
  ...
  Check other_steps_extend_trace.

  Lemma det_to_nondet {pick_sp : PickSp} f fpost s k t l mc :
   f O = Some (s, k, t, l, mc) ->
   possible_execution_nondet f ->
   (forall pick_sp',
       possible_execution_det (pick_sp := pick_sp') f ->
  ...
  Proof.
    ...
    intros terminates. simpl in terminates. specialize terminates with (1 := H1 _). fwd. exists i.
    rewrite E in *. destr_sstate p. subst. simpl. destruct terminates as [terminates|terminates].
    - right. assumption.
    - left. simpl in terminates. subst. split; [reflexivity|]. Check other_steps_extend_trace.
      specialize other_steps_extend_trace with (1 := H0). intros tail. specialize (tail O i ltac:(lia)).
      destruct tail as [tail|tail]; [congruence|]. rewrite E in *. fwd. simpl in tail. subst.
      rewrite H. simpl. eexists. split; [reflexivity|]. intros Hpred. enough (H2: possible_execution_det f).
      { rewrite H in E.  simpl in E. eapply satisfies_short; eauto. apply H1. assumption. }
      eapply det_to_nondet'; eauto. intros j. assert (Hor : j <= i \/ i < j) by lia. destruct Hor as [Hle|Hlt].
      + specialize other_steps_extend_trace with (1 := H0). intros H'. specialize H' with (1 := Hle).
        destruct H' as [H'|H']; [congruence|].  rewrite E, H in H'. simpl in H'. fwd.
        specialize (other_steps_extend_trace) with (1 := H0). intros H''. specialize (H'' O j ltac:(lia)).
  ...
  Lemma det_equiv_nondet s k t l mc fpost :
    excluded_middle ->
    (forall pick_sp f,
        f O = Some (s, k, t, l, mc) ->
        possible_execution_nondet (pick_sp := pick_sp) f ->
        satisfies_nondet (pick_sp := pick_sp) f (fun k' t' l' mc' =>
                                      exists k'',
                                        k' = k'' ++ k /\
                                          (compat (fun k_ => consume_word (pick_sp (rev k_ ++ k))) (List.rev k'') ->
                                           fpost pick_sp k' t' l' mc')))
  ...
  Lemma exec_det_equiv_nondet s k t l mc fpost :
    excluded_middle ->
    FunctionalChoice_on (option sstate) (option sstate) ->
    ext_spec.ok ext_spec ->
    word.ok word ->
    (forall pick_sp,
        exec_nondet pick_sp s k t l mc (fun k' t' l' mc' =>
                                    exists k'',
                                      k' = k'' ++ k /\
                                        (compat (fun k_ => consume_word (pick_sp (rev k_ ++ k))) (List.rev k'') ->
                                         fpost pick_sp k' t' l' mc')))
    <->
      (forall pick_sp,
          exec_det pick_sp s k t l mc (fpost pick_sp)).
  Proof.
    intros em choice ext_spec_ok word_ok. split.
